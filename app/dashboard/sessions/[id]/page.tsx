...
export default async function SessionDetailsPage({ params }: PageProps) {
  ...
  const totalVolume =
    session.session_exercises?.reduce((acc: number, curr: any) => {
        ...
        <CardContent className="pt-4 space-y-4 border-t border-slate-800">
          {session.notes && (
            <div className="bg-slate-800/40 rounded-lg p-3 border border-slate-700/30">
              <span className="text-slate-400 text-xs block font-semibold mb-1 uppercase tracking-wider">
                Ghi chú buổi tập
              </span>
          ...
          <div className="space-y-3">
            {session.session_exercises.map((ex: any, idx: number) => {
              ...
              return (
                <Card
                  key={ex.id}
                  className="bg-slate-900/30 border-slate-800 hover:border-slate-700/50 transition-colors"
                >
                  <CardContent className="p-4 space-y-3">
                    <div className="flex justify-between items-start">
                      <div>
                        <span className="text-orange-500/80 text-xs font-semibold uppercase tracking-wider block">
                          Bài {idx + 1}
                        </span>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {setsDetail.map((set: any, sIdx: number) => (
                            ...
                            >
                              ...
                              {set.completed && (
                                <span className="text-[10px] bg-orange-500/15 text-orange-400 px-1 rounded-sm">
                                  ✓
                                </span>
                              )}
                            </span>
