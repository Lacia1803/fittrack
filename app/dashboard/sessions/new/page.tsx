...
import { ArrowLeft, Plus, Trash2, Play, Pause, RotateCcw, Timer, Sparkles } from 'lucide-react'
    ...
    setExercises(prev => prev.map((ex, i) => {
      ...
      if (field === 'sets') {
        ...
        const newDetails = Array.from({ length: newSets }, (_, sIdx) => {
          return currentDetails[sIdx] || { reps: ex.reps || 10, weight_kg: ex.weight_kg || '', rpe: 8, completed: false }
        })
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between pb-2">
        ...
        <Link href="/dashboard" className="text-slate-400 hover:text-white text-xs underline">
          Quay lại dashboard
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          <Card className="premium-glass-card">
            <CardHeader>
              <CardTitle className="text-white text-md">Thông tin buổi tập</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label className="text-slate-300 text-xs">Tên buổi tập *</Label>
                  <Input
              ...
              {plans.length > 0 && (
                <div className="space-y-2">
                  <Label className="text-slate-300 text-xs">Workout Plan (tuỳ chọn)</Label>
                  <select
                    value={planId}
          ...
          <Card className="premium-glass-card">
            ...
            <CardContent className="space-y-6">
              {exercises.map((ex, index) => (
                <div key={index} className="bg-white/[0.02] border border-white/5 rounded-2xl p-4 space-y-4">
                  <div className="flex items-center justify-between">
                    <span className="text-orange-500 text-xs font-bold uppercase tracking-wider">Bài {index + 1}</span>
                    {exercises.length > 1 && (
                      <Button
                        variant="ghost"
                        size="icon"
                  ...
                  <div className="space-y-3 pt-3 border-t border-white/5">
                    <div className="flex justify-between items-center">
                      <span className="text-slate-400 text-xs font-bold uppercase tracking-wider block">Ghi từng hiệp</span>
                      <button
                        type="button"
                      ...
                      {ex.sets_detail.map((set, sIdx) => (
                        ...
                        >
                          <div className="col-span-1 text-center">
                            <span className="text-slate-500 text-xs font-bold font-mono">{sIdx + 1}</span>
                          </div>
                          ...
                          <div className="col-span-3 flex items-center gap-1.5">
                            <Input
                              ...
                              className="bg-slate-800 border-slate-700 text-white text-xs h-9 text-center font-mono placeholder:text-slate-600"
                            />
                            <span className="text-[10px] text-slate-500 font-mono">rep</span>
                          </div>
                          ...
                          <div className="col-span-2 flex items-center gap-1">
                            <Input
                              ...
                              max={10}
                              value={set.rpe}
                              onChange={(e) => updateSetDetail(index, sIdx, 'rpe', e.target.value ? parseInt(e.target.value) : '')}
                              className="bg-slate-800 border-slate-700 text-white text-xs h-9 text-center font-mono placeholder:text-slate-600"
                            />
                          </div>
                          ...
                          <div className="col-span-2 flex justify-end">
                            <button
                              type="button"
                              onClick={() => updateSetDetail(index, sIdx, 'completed', !set.completed)}
          ...
          <div className="flex gap-3">
            ...
            <Link href="/dashboard/sessions">
              <Button variant="outline" className="border-slate-800 bg-slate-900/40 hover:bg-slate-900 text-slate-300 px-8 py-5 rounded-full">
                Hủy
              </Button>
            </Link>
          </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="bg-gradient-to-b from-white/8 to-white/2 border border-white/12 rounded-[20px] p-5 shadow-xl backdrop-blur-md space-y-4">
            <strong className="text-white text-sm font-bold tracking-wider uppercase block">Tổng kết buổi tập</strong>
            
            <div className="space-y-3.5">
              <div className="flex justify-between items-center text-xs font-mono text-slate-400">
                <span>Tổng volume:</span>
                <strong className="text-white text-sm font-semibold">{sessionSummary.volume.toLocaleString('vi-VN')} kg</strong>
              </div>
              
              <div className="flex justify-between items-center text-xs font-mono text-slate-400">
                <span>Set hoàn thành:</span>
                <strong className="text-white text-sm font-semibold">{sessionSummary.completedSets}/{sessionSummary.totalSets}</strong>
              </div>
              ...
              <div className="flex justify-between items-start text-xs font-mono text-slate-400">
                ...
                <strong className="text-orange-500 font-semibold flex items-center gap-1 text-right shrink-0">
                  <Sparkles className="h-3 w-3" /> Giữ form, tăng tải +2.5kg tuần tới
                </strong>
              </div>
            </div>
          </div>
