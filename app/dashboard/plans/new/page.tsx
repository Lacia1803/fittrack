...
export default function NewPlanPage() {
  ...
  const handleGenerateName = async () => {
    ...
    try {
      const res = await fetch("/api/ai/suggest", {
        ...
        body: JSON.stringify({ type: "workout_plan_name", context: {} }),
      });
      const data = await res.json();
      if (data.suggestion) {
        setName(data.suggestion.replace(/[*#]/g, "").trim());
      } else {
  ...
  const handleGenerateDesc = async () => {
    if (!name) {
      toast({
        title: "Hãy nhập tên plan trước khi tạo mô tả",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      const res = await fetch("/api/ai/suggest", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ type: "workout_plan_desc", context: { name } }),
      });
      const data = await res.json();
      if (data.suggestion) {
  ...
  const handleCreate = async () => {
    ...
    if (error) {
      ...
      toast({
        title: "Thành công",
        description: "Tạo chương trình tập luyện mới thành công!",
      });
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-slate-400 mt-1 text-sm">
              Bắt đầu chặng đường thay đổi vóc dáng của bạn
            </p>
          </div>
        </div>
      </div>
          ...
          <Card className="premium-glass-card border-slate-700/50 shadow-xl overflow-hidden">
            ...
            <CardHeader className="pb-4">
              ...
              <CardDescription className="text-slate-400">
                Đặt tên và mô tả chi tiết chương trình tập của bạn để dễ dàng
                theo dõi.
              </CardDescription>
            </CardHeader>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    ...
                    className="h-7 text-xs text-purple-400 hover:text-purple-300 hover:bg-purple-900/20 px-2"
                  >
                    {generatingName ? (
            ...
            <CardContent className="space-y-4">
              <div className="flex gap-3 items-start">
                ...
                <div>
                  <p className="text-slate-200 font-medium text-sm">
                    Gợi ý cách chia lịch
                  </p>
                  <p className="text-slate-400 text-xs mt-1 leading-relaxed">
                    Bạn có thể tạo các plan theo cấu trúc phổ biến như{" "}
                    <strong>PPL</strong> (Push/Pull/Legs) cho 6 ngày/tuần, hoặc{" "}
                    <strong>Upper/Lower</strong> cho 4 ngày/tuần.
                  </p>
                </div>
              </div>
              ...
              <div className="flex gap-3 items-start">
                ...
                <div>
                  ...
                  <p className="text-slate-400 text-xs mt-1 leading-relaxed">
                    Nhóm các buổi tập vào chung một Plan giúp bạn dễ dàng theo
                    dõi sự tiến bộ về mức tạ (Progressive Overload) cực kỳ trực
                    quan.
                  </p>
                </div>
              </div>
              ...
              <div className="flex gap-3 items-start">
                ...
                <div>
                  ...
                  <p className="text-slate-400 text-xs mt-1 leading-relaxed">
                    Khai báo mô tả chi tiết, AI Coach sẽ đọc được những lưu ý
                    này và đưa ra những tư vấn chính xác hơn cho lộ trình riêng
                    của bạn.
                  </p>
                </div>
              </div>
