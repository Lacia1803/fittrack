  ...
  const handleSeed = async () => {
    ...
    try {
      ...
      await supabase.from("session_exercises").insert([
        {
          session_id: s1.id,
          exercise_name: "Back Squat (Gánh đùi)",
          sets: 4,
          reps: 8,
          weight_kg: 80,
          notes: "Hiệp cuối hơi mệt",
        },
        {
          session_id: s1.id,
          ...
          reps: 10,
          weight_kg: 60,
          notes: "Giữ lưng thẳng cực tốt",
        },
        {
          session_id: s1.id,
          exercise_name: "Leg Press (Đạp đùi)",
          sets: 3,
          reps: 12,
          weight_kg: 120,
          notes: "Pump đùi trước tối đa",
        },
      ]);
      ...
      await supabase.from("session_exercises").insert([
        {
          session_id: s3.id,
          exercise_name: "Barbell Row (Gập người chèo tạ)",
          sets: 4,
          reps: 8,
          weight_kg: 50,
          notes: "Tập trung kéo cùi chỏ về sau",
        },
        {
          session_id: s3.id,
          exercise_name: "Lat Pulldown (Kéo xô máy)",
          sets: 3,
          reps: 10,
          weight_kg: 45,
          notes: "Căng xô tối đa ở đỉnh",
        },
        {
          session_id: s3.id,
          exercise_name: "Bicep Curl (Cuốn tạ đơn)",
          sets: 3,
          reps: 12,
          weight_kg: 12,
          notes: "Bơm căng tay trước",
        },
      ]);

      // 3. Tạo Milestones mẫu (theo timeline hợp lý)
      const milestones = [
        {
          title: "Khởi động lại thói quen",
          description: "Bắt đầu hành trình tập luyện đều đặn.",
          is_achieved: true,
          achieved_at: format(subDays(today, 6), "yyyy-MM-dd"),
          user_id: user.id,
        },
        {
          title: "Buổi tập đầu tiên",
          description: "Log thành công buổi tập đầu tiên trên FitTrack!",
          is_achieved: true,
          achieved_at: format(subDays(today, 5), "yyyy-MM-dd"),
          user_id: user.id,
        },
        {
          title: "Ghi log dinh dưỡng ngày đầu",
          description: "Hoàn thành ngày đầu theo dõi calo và macros.",
          is_achieved: true,
          achieved_at: format(subDays(today, 4), "yyyy-MM-dd"),
          user_id: user.id,
        },
        {
          title: "Chinh phục Bench Press 75kg",
          description: "Đẩy ngực ngang thành công với mức tạ 75kg!",
          is_achieved: true,
          achieved_at: format(subDays(today, 3), "yyyy-MM-dd"),
          user_id: user.id,
        },
        {
          title: "Ảnh tiến trình đầu tiên",
          description: "Lưu ảnh tiến trình để so sánh sau này.",
          is_achieved: true,
          achieved_at: format(subDays(today, 2), "yyyy-MM-dd"),
          user_id: user.id,
        },
        {
          title: "Kỷ luật Thép",
          description: "Hoàn thành 3 buổi tập trong 1 tuần.",
          is_achieved: true,
          achieved_at: format(subDays(today, 1), "yyyy-MM-dd"),
          user_id: user.id,
        },
        {
          title: "Mục tiêu Bench Press 100kg",
          description: "Đẩy ngực ngang mức tạ 100kg.",
          is_achieved: false,
          user_id: user.id,
        },
        {
          title: "Chuỗi 30 ngày kỷ luật",
          description: "Hoàn thành 30 ngày theo dõi tập luyện liên tục.",
          is_achieved: false,
          user_id: user.id,
        },
      ];
      ...
      const progressPhotos = [
        {
          ...
          caption: "Ngày bắt đầu FitTrack - Cân nặng 72kg",
          taken_at: format(subDays(today, 5), "yyyy-MM-dd"),
          user_id: user.id,
        },
        {
          photo_url:
            "https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?w=500",
          caption: "Sau 5 ngày tập luyện hăng say",
          taken_at: format(today, "yyyy-MM-dd"),
          user_id: user.id,
        },
      ];
      await supabase.from("progress_photos").insert(progressPhotos);

      // 5. Tạo dữ liệu dinh dưỡng mẫu (localStorage)
      if (typeof window !== "undefined") {
        ...
        nutritionSamples.forEach((sample) => {
          localStorage.setItem(
            `nutrition_${sample.date}`,
            JSON.stringify({
      ...
      toast({
        ...
        description:
          "3 plans, 3 buổi tập lịch sử, 8 cột mốc, 2 ảnh tiến trình và dữ liệu dinh dưỡng mẫu đã được thiết lập.",
      });
      ...
      toast({
        ...
        description:
          error.message || "Vui lòng kiểm tra lại quyền hoặc database.",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
