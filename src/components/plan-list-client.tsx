...
export default function PlanListClient({
  ...
  const handleDeleteSelected = async () => {
    ...
    if (
      !confirm(
        `Bạn có chắc muốn xóa ${selectedIds.size} Plan đã chọn? Các buổi tập thuộc Plan này cũng sẽ mất liên kết.`,
      )
    )
      return;
    ...
    if (error) {
      toast({
        title: "Lỗi",
        description: "Không thể xóa các Plan đã chọn.",
        variant: "destructive",
      });
    } else {
      toast({
        title: "Thành công",
        description: `Đã xóa ${selectedIds.size} Plan.`,
      });
