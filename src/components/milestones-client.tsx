...
export default function MilestonesClient({
  ...
  const milestones: Milestone[] = [
    ...
    {
      label: "Sát thủ phòng gym (200 buổi)",
      target: 200,
      current: sessionCount,
      icon: Crown,
    ...
    {
      label: "Kỷ luật thép (1 năm / 365 buổi)",
      target: 365,
      current: sessionCount,
      icon: Crown,
    ...
    {
      label: "Huấn luyện viên (10 plans)",
      target: 10,
      current: planCount,
      icon: CalendarDays,
    ...
    {
      label: "Hành trình lột xác (10 ảnh)",
      target: 10,
      current: photoCount,
      icon: Camera,
    ...
    {
      label: "Minh chứng rõ rệt (20 ảnh)",
      target: 20,
      current: photoCount,
      icon: Camera,
      color: "text-cyan-500",
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {milestones.map((m) => {
        ...
        const displayCurrent = Math.min(m.current, m.target);
        return (
          <Card
            key={m.label}
            className={cn(
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <m.icon
                    className={cn(
                      "h-5 w-5",
                      achieved ? m.color : "text-slate-500",
                    )}
                  />
                  <span
                    className={cn(
                      "font-medium text-sm",
                      achieved ? "text-white" : "text-slate-400",
                    )}
                  >
                    {m.label}
                  </span>
              ...
              <div className="w-full bg-slate-700 rounded-full h-2">
                <div
                  className={cn(
                    ...
                    achieved ? "bg-orange-500" : "bg-slate-500",
                  )}
                  style={{ width: `${progress}%` }}
                />
              </div>
              <p className="text-slate-500 text-xs text-right">
                {displayCurrent}/{m.target}
              </p>
            </CardContent>
          </Card>
        );
