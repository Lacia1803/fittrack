...
export default function DashboardClient({
    ...
    return (
      session.session_exercises?.reduce((sum, ex) => {
        ...
        if (setsDetail) {
          return (
            sum +
            setsDetail.reduce((sSum: number, s: any) => {
  ...
  const stats = useMemo(() => {
    ...
    const currentSessions = initialSessions.filter(
    ...
    const currentVolume = currentSessions.reduce(
      (sum, s) => sum + getSessionVolume(s),
      0,
    );
    const prevVolume = prevSessions.reduce(
      (sum, s) => sum + getSessionVolume(s),
      0,
    );
    ...
    const compliancePct = Math.min(
      Math.round((currentSessions.length / targetSessions) * 100),
      100,
    );
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
        <div>
          ...
          <p className="text-slate-400 mt-1">
            Tuần này · {stats.sessionCount} buổi tập · Mục tiêu tăng cơ
          </p>
        </div>
        ...
        <Card className="premium-glass-card hover:border-orange-500/30">
          ...
          <CardContent>
            <p className="text-3xl font-bold text-white font-mono">
              {stats.currentVolume.toLocaleString("vi-VN")} kg
            </p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <Card className="premium-glass-card lg:col-span-8">
          <CardHeader>
            <CardTitle className="text-white text-md font-semibold">
              Biểu đồ khối lượng & tăng trưởng tạ
            </CardTitle>
            <CardDescription className="text-slate-400 text-xs">
              Vẽ tiến trình tập luyện theo bộ lọc thời gian
            </CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card className="premium-glass-card lg:col-span-4 flex flex-col justify-between">
          <CardHeader>
            ...
            <CardDescription className="text-slate-400 text-xs">
              Theo dõi chất dinh dưỡng vi lượng
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-5">
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between text-xs">
                ...
                <span className="text-white font-bold font-mono">
                  {nutrition.protein}g / {targetProt}g
                </span>
              </div>
              <div className="h-2 w-full bg-white/5 rounded-full overflow-hidden">
                <div
                  ...
                  style={{
                    width: `${getProgressPct(nutrition.protein, targetProt)}%`,
                  }}
                />
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between text-xs">
                ...
                <span className="text-white font-bold font-mono">
                  {nutrition.carbs}g / {targetCarbs}g
                </span>
              </div>
              <div className="h-2 w-full bg-white/5 rounded-full overflow-hidden">
                <div
                  className="h-full bg-orange-500 rounded-full transition-all duration-500"
                  style={{
                    width: `${getProgressPct(nutrition.carbs, targetCarbs)}%`,
                  }}
                />
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="h-2 w-full bg-white/5 rounded-full overflow-hidden">
                <div
                  className="h-full bg-orange-500 rounded-full transition-all duration-500"
                  style={{
                    width: `${getProgressPct(nutrition.fats, targetFats)}%`,
                  }}
                />
              </div>
            </div>
        ...
        <div className="lg:col-span-4 space-y-4">
          ...
          <div className="flex flex-col gap-3">
            ...
            <Link href="/dashboard/plans/new">
              <Button
                variant="outline"
                className="w-full border-slate-700 hover:bg-slate-800 text-slate-300 py-5 rounded-xl"
              >
            ...
            {stats.currentSessions.length === 0 ? (
              <div className="p-8 text-center border border-dashed border-slate-800 rounded-xl bg-slate-900/10">
                <p className="text-slate-500 text-xs sm:text-sm">
                  Chưa có buổi tập nào được ghi nhận trong khoảng thời gian này.
                </p>
              </div>
            ) : (
              stats.currentSessions.slice(0, 4).map((s) => {
                ...
                return (
                  <div
                    key={s.id}
                    className="p-4 rounded-xl border border-white/5 bg-white/[0.02] flex items-center justify-between hover:border-orange-500/20 transition-all"
                  >
                    ...
                    <div className="text-right">
                      <span className="text-white text-xs sm:text-sm font-bold font-mono">
                        {vol.toLocaleString("vi-VN")} kg
                      </span>
                      <span className="text-[10px] text-orange-500 block font-medium mt-0.5">
                        +4.2% volume
                      </span>
                    </div>
                  </div>
